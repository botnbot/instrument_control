...
class Instruments(models.Model):
    ...
    is_new = models.BooleanField(
        default=True
    )  # не используется. Для пометки, что инструмент был приобретён новым
    ...
    def total_repair_duration(self):
        return sum(r.duration or 0 for r in self.repairs.all())
...
class Repairers(models.Model):
    ...
    address = models.CharField(
        max_length=300, verbose_name="Адрес", null=True, blank=True
    )
    phone = models.CharField(max_length=20, verbose_name="Телефон")
    contact_person = models.CharField(
        max_length=100, verbose_name="Контактное лицо", null=True, blank=True
    )
    email = models.EmailField(null=True, blank=True)
    company_details = models.JSONField(
        default=dict, blank=True, verbose_name="Реквизиты"
    )
    comment = models.CharField(
        max_length=300, verbose_name="Комментарий", null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not is_new:
            ...
            try:
                ...
                if (
                    old.date_of_receipt_from_repair is None
                    and self.date_of_receipt_from_repair is not None
                ):
        ...
        if (
            is_new
            and self.instrument
            and self.instrument.status != Instruments.STATUS_IN_REPAIR
        ):
...
class Relocations(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.instrument.status not in [
            Instruments.STATUS_RELOCATED,
            Instruments.STATUS_IN_REPAIR,
        ]:
